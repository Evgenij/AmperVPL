...
void __fastcall Tlab_inf::Image3Click(TObject *Sender)
{
	...
	{
		count=1;
		FProjects->memo->Lines->Add("ЛАБОРАТОРНАЯ РАБОТА №1");
		FProjects->memo->Lines->Add("Тема: Электрический ток в металлах. Законы постоянного тока.");
		FProjects->memo->Lines->Add("Цель: Проверить формулы эквивалентного сопротивления при последовательноми и параллельном соединениях проводников");
		FProjects->memo->Lines->Add("---------------------------------");
		FProjects->memo->Lines->Add("Тип соединения: последовательное");
		FProjects->memo->Lines->Add("---------------------------------");
		FProjects->memo->Lines->Add("------------Ход работы-----------");
	}

	FProjects->lab=new TMain(Main);
	FProjects->lab->Caption="Лабараторная работа №1 - Электрический ток в металлах. Законы постоянного тока";
	lab_inf->Close();
	FProjects->lab->Show();
	...
	FValue->SG->Cells[2][0]="R1, Ом";
	FValue->SG->Cells[3][0]="R2, Ом";
	FValue->SG->Cells[4][0]="U R1,R2, В";
	FValue->SG->Cells[5][0]="I, А";
	FValue->SG->Cells[6][0]="R, Ом";
	FValue->SG->Cells[7][0]="P, Вт";
...
void __fastcall Tlab_inf::Image1Click(TObject *Sender)
{
	...
	if(FProjects->memo!=NULL)
	{
		...
		FProjects->memo->Lines->Add("Цель: Проверить формулы эквивалентного сопротивления при последовательноми и параллельном соединениях проводников");
		FProjects->memo->Lines->Add("---------------------------------");
		FProjects->memo->Lines->Add("Тип соединения: параллельное");
		FProjects->memo->Lines->Add("---------------------------------");
		FProjects->memo->Lines->Add("------------Ход работы-----------");
	}
	...
	FValue->SG->Cells[2][0]="R1, Ом";
	FValue->SG->Cells[3][0]="R2, Ом";
	FValue->SG->Cells[4][0]="R3, Ом";
	FValue->SG->Cells[5][0]="U R1,R2,R3, В";
	FValue->SG->Cells[6][0]="I, А";
	FValue->SG->Cells[7][0]="R, Ом";
	FValue->SG->Cells[8][0]="P, Вт";
